...
harts through \Fhaltreq, \Fresumereq, \Fresethaltreq and \Fhartreset in \Rdmcontrol.
...
The debugger can provide an optional \Fresethaltreq bit. Writing to this bit
sets or clears the halt-on-reset request bits for each selected hart. When
a hart's halt-on-reset request bit is set, the hart will immediately enter
debug mode on the next deassertion of its reset. This is true regardless of
whether the hart's reset is not caused by the debugger's
\Fndmreset/\Fhartreset signals.
The hart's halt-on-reset request bit remains set
until explicitly cleared by the debugger writing \Fresethaltreq
to 0 while the hart is selected.
